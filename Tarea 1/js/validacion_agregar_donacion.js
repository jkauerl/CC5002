...
const validarForm = () => {
    const validadorLargoMax = (texto, largo) => texto && texto.length <= largo;
    const validadorLargoMin = (texto, largo) => texto && texto.length >= largo;
    const validadorMail = (mail) => mail && mail.includes("@") && validadorLargoMin(mail, 5);
    const validadorFecha = (fecha) => {
        let fechaArr = fecha.split('-');
        if(fechaArr[0].length == 4 && fechaArr[1].length == 2 && fechaArr[2].length == 2 && fechaArr.length == 3) {
            let dia_actual = new Date();
            let dia_ingresado = new Date(parseInt(fechaArr[0]), parseInt(fechaArr[1]) - 1, parseInt(fechaArr[2]));
    ...
    const validadorCelular = (numero) => {
        let patronCelularChile = /^(?:\+?56|0)[2-9]\d{1}\s?\d{3}\s?\d{4}$/;
        return patronCelularChile.test(numero);
    }

    let regionInput = document.getElementById("region");
    let comunaInput = document.getElementById("comuna");
    let calle_numeroInput = document.getElementById("calle-numero");
    ...
    if (!validadorSelect(regionInput)){
    ...
    if (!validadorSelect(comunaInput)){
        msg += "Seleccionar Comuna\n";
        isValid = false;
    ...
    if (!validadorLargoMax(calle_numeroInput.value, 80)){
        msg += "Calle y Número entre 1 y 80 caracteres\n";
        isValid = false;
    ...
    if (!validadorSelect(tipoInput)){
        msg += "Seleccionar Tipo de Donación\n";
        isValid = false;
    ...
    if (!validadorLargoMax(cantidadInput.value, 250)){
        msg += "Agregar Cantidad\n";
        isValid = false;
    ...
    if (foto1.value === "" && foto2.value === "" && foto3.value === "") {
        msg += "Agregar por lo menos una foto\n";
        isValid = false;
        foto1.style.outlineColor = "red";
    }

    if(!validadorLargoMin(nombreInput.value, 3) && !validadorLargoMax(nombreInput.value, 80)) {
        msg += "Agregar nombre entre 3 y 80 caracteres\n";
        isValid = false;
    ...
    if(!validadorCelular(celularInput.value)) {
        msg += "Agregar un celular correcto\n";
        isValid = false;
...
const confirmacionFormulario = () => {
    ocultarMensaje(formularioDiv);
    ocultarMensaje(mensajeConfirmacion);
...
const crearComunas = () => {
    for(let i in regiones){
        for(let j in regiones[i].comunas){
            let opcion = document.createElement("option");
            opcion.textContent = regiones[i].comunas[j].nombre;
            opcion.value = regiones[i].comunas[j].id;
            selectComuna.append(opcion);
        }
    }
};
crearRegiones();
...
const cambiarRegiones = (id) => {
    for(let i = selectRegion.options.length -1; i>0; i--) {
        selectRegion.remove(i);
    }
    for(let i in regiones) {
        for(let j in regiones[i].comunas) {
...
const cambiarComunas = (id) => {
    for(let i = selectComuna.options.length -1; i>0; i--) {
        selectComuna.remove(i);
    }
    for(let i in regiones) {
        if(regiones[i].id == id) {
            for(let j in regiones[i].comunas) { 
...
selectRegion.addEventListener("change", (event) => cambiarComunas(event.selectRegion.value));

selectComuna.addEventListener("change", (event) => cambiarRegiones(event.selectComuna.value));

const anularFormulario = () => {
    mostrarMensaje(formularioDiv);
    ocultarMensaje(mensajeConfirmacion);
};

let formularioDiv = document.getElementById("formulario-div");
let submitBtn = document.getElementById("envio-donacion");
submitBtn.addEventListener("click", validarForm);

let mensajeConfirmacion = document.getElementById("mensaje-confirmacion");
let botonConfirmacionFormulario = document.getElementById("confimar-formulario");
let botonAnularFormulario = document.getElementById("anular-formulario");
let mensajeRecibido = document.getElementById("mensaje-recibido");

botonConfirmacionFormulario.addEventListener('click', confirmacionFormulario);

botonAnularFormulario.addEventListener('click', anularFormulario);
